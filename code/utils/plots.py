...
def plot_normal_image(rgb, path, img_index, plot_nrow, img_res, type, fill=True):
    rgb_plot = lin2img(rgb, img_res)
    ...
    scale_factor = 255
    tensor = (tensor + 1) / 2
    # mask = (tensor[:, :, 0] <= 0.01) and (tensor[:, :, 1] <= 0.01) and (tensor[:, :, 2] <= 0.01)
    # mask1 = (tensor[:, :, 0] <= 0.05)
    # mask2 = (tensor[:, :, 1] <= 0.05)
    # mask3 = (tensor[:, :, 2] <= 0.05)
...
def plot_images(model_outputs, ground_truth, path, epoch, img_index, plot_nrow, img_res, batch_size, is_eval):
    ...
    if 'normal_image' in model_outputs:
        ...
        normal_points[:, :, 0] *= -1
        normal_points[:, :, 2] *= -1
        # a = normal_points.cpu().numpy()

    if 'canonical_normal' in model_outputs:
        normalc_points = model_outputs['canonical_normal']
        normalc_points = normalc_points.reshape(batch_size, num_samples, 3)
        normalc_points[:, :, 1] *= -1
        normalc_points[:, :, 0] *= -1
        normalc_points[:, :, 2] *= -1
    ...
    if rgb_gt is not None:
        if 'normal_image' in model_outputs:
            output_vs_gt = torch.cat((output_vs_gt, rgb_gt, normal_points, normalc_points), dim=0)
        else:
            output_vs_gt = torch.cat((output_vs_gt, rgb_gt), dim=0)
    else:
        if 'normal_image' in model_outputs:
            output_vs_gt = torch.cat((output_vs_gt, normal_points, normalc_points), dim=0)
        else:
    ...
    if is_eval:
        for i, idx in enumerate(img_index):
            ...
            # plot_image(model_outputs['shading_image'].reshape(batch_size, num_samples, 3)[[i]], path[i], idx, plot_nrow, img_res, 'shading')
            # plot_normal_image(normal_points[[i]], path[i], idx, plot_nrow, img_res, 'normal', fill=False)
            # plot_normal_image(normalc_points[[i]], path[i], idx, plot_nrow, img_res, 'normalc', fill=False)
            # filename = path[i] + 'ply/{0}_canonical_verts.ply'.format(img_index)
            # save_pcl_to_ply(filename, model_outputs['deformed_points'].reshape(-1, 3), colors=None)
